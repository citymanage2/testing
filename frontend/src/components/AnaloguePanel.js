...
export default function AnaloguePanel({ taskId, itemId, isAnalogue, onClose, onApplied }) {
    ...
    async function apply(analogue) {
        setApplying(analogue.id);
        try {
            await client.post(`${base}/apply-analogue`, { price: analogue.price, source_url: analogue.source_url });
            onApplied();
        }
        catch {
